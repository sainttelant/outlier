...
*/
{
	...
	CPoint normalizedInstance(color,n_InstanceDimension);	// 初始化CPoint类，数值为随机
...
*/
{
	...
	float outlierscolor[3] = {0.9f,0.f,0.f};
	float normalcolor[3] = { 0.0f,0.9f,0.0f };
	for (int i = 0; i < vec_Instances.size(); i++)
	{
		...
		if (value>1.0)
		{
			instance.Setcolor(outlierscolor);
			vec_Instances.at(i).Setcolor(outlierscolor);
			outlier temp = { value, instance, i };	// { lof, instance, index }
			vec_Outliers.push_back(temp);
		}
		else
		{
			vec_Instances.at(i).Setcolor(normalcolor);
		}
	}
